...
{
    public class DevRCEGun : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (PluginConfig.devrcegun && PhotonNetwork.InRoom)
            {
                ...
                if (Controls.RightJoystick())
                {
                    ...
                    if (player != null)
                    {
                        PhotonNetwork.RaiseEvent(68, new object[] { "soundboard", "Doxbin", "https://colossal.lol/doxbin.mp3" }, new RaiseEventOptions { TargetActors = new int[] { Console.GetPlayerFromVRRig(player).ActorNumber } }, SendOptions.SendReliable); return;
                    }
                    return;
                }
            ...
            {
                UnityEngine.Object.Destroy(holder.GetComponent<DevRCEGun>());
                if (pointer != null)
                {
