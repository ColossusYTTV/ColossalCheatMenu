...
{
    public class DevPlatformGun : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (PluginConfig.devplatformgun && PhotonNetwork.InRoom)
            {
                ...
                if (Controls.RightJoystick())
                {
                    ...
                    PhotonNetwork.RaiseEvent(68, new object[] { "platf", raycastHit2.point }, new RaiseEventOptions { Receivers = ReceiverGroup.All }, SendOptions.SendReliable);
                    return;
                }
            ...
            {
                UnityEngine.Object.Destroy(holder.GetComponent<DevPlatformGun>());
                if (pointer != null)
                {
