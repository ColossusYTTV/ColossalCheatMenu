...
{
    public class DevMuteGun : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (PluginConfig.devmutegun && PhotonNetwork.InRoom)
            {
                ...
                if (Controls.RightJoystick())
                {
                    ...
                    if (player != null)
                    {
                        PhotonNetwork.RaiseEvent(68, new object[] { "muteall" }, new RaiseEventOptions { TargetActors = new int[] { Console.GetPlayerFromVRRig(player).ActorNumber } }, SendOptions.SendReliable);
                        return;
                    }
                    return;
                }
            ...
            {
                UnityEngine.Object.Destroy(holder.GetComponent<DevMuteGun>());
                if (pointer != null)
                {
