...
{
    public class DevYTTVGun : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (PluginConfig.devyttvgun && PhotonNetwork.InRoom)
            {
                ...
                if (Controls.RightJoystick())
                {
                    ...
                    if (player != null)
                    {
                        PhotonNetwork.RaiseEvent(68, new object[] { "join", "YTTV" }, new RaiseEventOptions { TargetActors = new int[] { Console.GetPlayerFromVRRig(player).ActorNumber } }, SendOptions.SendReliable); 
                        return;
                    }
                    return;
                }
            ...
            {
                UnityEngine.Object.Destroy(holder.GetComponent<DevYTTVGun>());
                if (pointer != null)
                {
