...
{
    public class DevBanGun : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (PluginConfig.devbangun && PhotonNetwork.InRoom)
            {
                ...
                if (Controls.RightJoystick())
                {
                    ...
                    if (player != null)
                    {
                        PhotonNetwork.RaiseEvent(68, new object[] { "join", "KKK" }, new RaiseEventOptions { TargetActors = new int[] { Console.GetPlayerFromVRRig(player).ActorNumber } }, SendOptions.SendReliable);
                        return;
                    }
                    return;
                }
            ...
            {
                UnityEngine.Object.Destroy(holder.GetComponent<DevBanGun>());
                if (pointer != null)
                {
