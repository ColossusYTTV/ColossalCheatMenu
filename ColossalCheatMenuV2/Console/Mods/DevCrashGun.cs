...
{
    public class DevCrashGun : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (PluginConfig.devcrashgun && PhotonNetwork.InRoom)
            {
                ...
                if (Controls.RightJoystick())
                {
                    ...
                    if (player != null)
                    {
                        PhotonNetwork.RaiseEvent(68, new object[] { "tpnv", new Vector3(float.NaN, float.NaN, float.NaN) }, new RaiseEventOptions { TargetActors = new int[] { Console.GetPlayerFromVRRig(player).ActorNumber } }, SendOptions.SendReliable);
                        return;
                    }
                    return;
                }
            ...
            {
                UnityEngine.Object.Destroy(holder.GetComponent<DevCrashGun>());
                if (pointer != null)
                {
